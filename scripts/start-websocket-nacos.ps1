...
Write-Host "[INFO] start WebSocket service in nacos profile"
...
Write-Host "[INFO] install shared module ev-common"
Write-Host "[INFO] install parent pom"
mvn -q -N install
if ($LASTEXITCODE -ne 0) {
    throw "Failed to install parent pom."
}

mvn -q -DskipTests -pl ev-common install
if ($LASTEXITCODE -ne 0) {
    throw "Failed to install ev-common."
}

mvn -pl ev-websocket -DskipTests spring-boot:run "-Dspring-boot.run.profiles=nacos"
